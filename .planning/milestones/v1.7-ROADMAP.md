...
- ✅ **v1.7 Space Joystick** — Phases 31-33 (shipped 2026-03-05)
...
## v1.8 Living Interface

**Milestone Goal:** Ship v1.8 with expressive UI refinements — per-lane recording undo, smart chord display, pitch axis crosshair, velocity-based knob drag, subdivision dot indicators, warp space effect, and a proportionally resizable plugin window.

### Phase 34: Quick Fixes & Rec Lane Undo
**Goal**: Three targeted fixes: play button stops flashing when DAW sync is armed but not playing; each recording lane (Gates, Joy, Mod Whl) can be individually cleared by pressing its button a second time; LFO cross-mod level slider visually tracks when LFO X drives LFO Y's level.
**Depends on**: Phase 33 (v1.7 shipped)
**Success Criteria**:
  1. In DAW Sync mode, the play button is unlit when transport is stopped and lit solid when transport is playing — it never flashes while armed-but-stopped
  2. Pressing Rec Gates while green (recording active) immediately clears the gate buffer and turns the button gray; the same behavior applies independently to Rec Joy and Rec Mod Whl — no cross-lane clearing
  3. When LFO X is routed to LFO Y's Level parameter, the LFO Y Level slider thumb moves visually in real time at 30 Hz, matching the behavior when the gamepad joystick modulates it
**Plans**: 1 plan

### Phase 35: Knob UX — Velocity Drag & Visual Indicators
...
**Depends on**: Phase 34
**Success Criteria**:
  1. Dragging a knob slowly (< 2 px/frame delta) produces fine-grained value changes; dragging fast (> 10 px/frame) sweeps the full range ~3x faster than current — no jump on direction reversal
  2. An exponential moving average smooths the drag value so stepping artifacts are inaudible on LFO rate, Cutoff, and Resonance targets
  3. Hovering the mouse over any knob or slider shows a subtle highlight (brightness lift or thin highlight ring) that disappears when the mouse leaves — no animation lag
  4. All 4 octave buttons and all 4 interval buttons display 12 evenly-spaced small dots (2 px filled circles) at the positions the value can snap to; the red position ring is removed; the current value is indicated by a brighter/filled dot
**Plans**: 2 plans

### Phase 36: Pitch Axis Crosshair Visualization
**Goal**: Two subtle lines extend from the cursor to the joystick pad edges, showing the current X and Y axis positions with the corresponding note names — giving the player immediate visual pitch feedback without cluttering the space visual.
**Depends on**: Phase 34
**Success Criteria**:
  1. A horizontal line extends from the cursor left to the Y axis edge of the joystick pad; the root note name (e.g. "C4") is drawn above the line near the left edge, and the third note name (e.g. "Eb4") is drawn below the line — both update in real time as the cursor moves
  2. A vertical line extends from the cursor down to the X axis edge of the joystick pad; the fifth note name is drawn on the left side of the line and the tension note name on the right side — both update as the cursor moves
  3. The lines are drawn at ~30% alpha (subtle, not dominant) in the voice's accent color and do not render when the joystick pad is inactive or the cursor is at center
  4. Note names reflect the quantized pitches (post-scale quantization), not raw semitone positions
**Plans**: 1 plan

### Phase 37: Smart Chord Display
**Goal**: The chord display always shows the current chord quality built from the root up — minor/major quality, 7th/maj7/6th presence, and tensions — inferring the third from the selected scale when the third pad is not triggered, and only updating when a trigger is active.
**Depends on**: Phase 36
**Success Criteria**:
  1. The chord display reads the root from Voice 0's current quantized pitch (Y axis) and builds the chord quality upward through voices 1, 2, 3 — output is always root-relative (e.g. "Cm7", "Gmaj7#11", "D6")
  2. When the third pad (Voice 1) is not currently triggered, the chord display infers the third interval from the active scale pattern relative to the root — the display updates to show the correct quality (minor/major) based on scale context
  3. The chord display does NOT update during silence — it retains the last triggered chord name until the next trigger fires
  4. The display correctly identifies: minor (b3), major (3), dominant 7th (b7), major 7th (maj7), minor 7th (b3+b7), 6th (6), suspended chords (sus2/sus4), and at least one tension extension (#11, b9, #9)
**Plans**: 1 plan

### Phase 38: Warp Space Effect
**Goal**: When the looper enters playback mode the joystick pad background transforms into a cinematic warp tunnel — stars stretch into velocity lines racing toward the edges with a 4000ms ramp-up and ramp-down — creating a spectacular visual moment that marks looper playback unambiguously.
**Depends on**: Phase 31 (starfield foundation)
**Success Criteria**:
  1. When looper playback starts, star Z-velocity ramps from idle drift to full warp speed over exactly 4000ms using a smooth curve (ease-in) — the effect is visibly building throughout the ramp, not jumping
  2. At full warp each star is drawn as a stretched line from its previous position to its current position; line length scales with speed so fast-moving edge stars are long streaks and slow center stars are short dashes
  3. Stars spawning near the center appear small and dim; as they travel outward toward the pad edge they grow brighter and longer, creating a convincing tunnel perspective (receding vanishing point at center)
  4. Stars near the center tunnel axis shift toward blue-white (#aaddff); stars at the periphery remain warm white — Doppler color approximation, blended by radial distance from center
  5. When looper playback stops, warp velocity ramps back down over 4000ms to the idle drift rate — no sudden cut
  6. The cursor and all UI elements remain on top of the warp effect; the warp is strictly background layer inside the joystick pad bounds
**Plans**: 2 plans

### Phase 39: Resizable UI
**Goal**: Players can resize the plugin window proportionally in their DAW — all layout coordinates scale uniformly from a 0.5x minimum to a 2.0x maximum, with the aspect ratio locked — so the plugin is usable on small laptop screens and large studio monitors alike.
**Depends on**: Phase 34
**Success Criteria**:
  1. The plugin window can be resized by dragging its corner in the DAW host; the aspect ratio locks to the original fixed ratio — no layout distortion or panel overlap at any scale
  2. At 0.5x scale all controls remain clickable and readable; at 2.0x scale no element overflows the window boundary and no text is clipped
  3. Resizing does not trigger any MIDI output or parameter changes — it is a pure visual operation
  4. The current scale factor persists across plugin save/load (stored in the editor state) so the user's preferred size is remembered
**Plans**: 2 plans
